...
const TrendingCard = ({ className, post }: TrendingCardProps) => {
    return (
        ...
        href={`${process.env.NEXT_PUBLIC_URL}/post/${post?.id}`} >
            {/* fill for image only working if classname is relative */}
            ...
            {/* gradient effect relative to parent element */}
  ...
  return (
    <section className='pt-3 pb-10'>
        <div className='flex items-center gap-3'>
            <div className='bg-wh-900 py-2 px-8 text-wh-10 text-sm font-bold'>NEW ISSUES</div>
            {/* <p className=' border-4 m-3 italic'>
                Some new articles
            </p> */}
        </div>
...
{/* flex */}
        ...
        <div className='flex flex-col basis-1/2 h-96 gap-3'>
            ...
            <div className='flex basis-1/2 bg-wh-500 gap-3'>
                <div className='basis-1/2 bg-wh-500'></div>
            </div>
        </div>
        </div> */}
        ...
        <p className='text-sm'>
            loremniusjgadg adgadogjag sadjgdsajga adsgj 
        </p>
    </section>
  )
}
